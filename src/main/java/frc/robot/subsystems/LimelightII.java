...
public class LimelightII {
  ...
  */
  ...
  public double steeringAssist(SwerveSubsystem dt) {
    tv = NetworkTableInstance.getDefault().getTable("limelight").getEntry("tv").getDouble(0.0);
    ...
    if (Math.abs(tx) < 1.0 && Math.abs(prev_tx) < 1.0 && Math.abs(dt.getHeading().getDegrees() - prevHeading) < 1) stopSteer = true;
    else stopSteer = false;
    if(stopSteer) {
      adjustment = 0;
    }
    prevHeading = dt.getHeading().getDegrees();
  ...
  public double[] autoTarget(SwerveSubsystem dt) {
